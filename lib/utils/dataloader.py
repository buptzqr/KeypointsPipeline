...
from dataset.presentDataset import PresentDataset
...
def get_present_loader(cfg, num_gpu, local_rank, file_path, is_dist=True):
    # -------- get raw dataset interface -------- #
    normalize = transforms.Normalize(mean=cfg.INPUT.MEANS, std=cfg.INPUT.STDS)
    transform = transforms.Compose([transforms.ToTensor(), normalize])
    attr = load_dataset(cfg.DATASET.NAME)
    dataset = PresentDataset(attr, file_path, transform)
