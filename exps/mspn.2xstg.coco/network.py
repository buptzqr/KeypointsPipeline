...
class ResNet_downsample_module(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, efficient=False):
        # block这里就是bottleneck
        # bottleNeck输出的维度planes*block.expansion,downSample应当和bottleNeck输出的维度一致
        # inplanes代表上一层给本层的输入维度，planes*block.expansion代表本层期望的一个输出维度
        # 上层的输入需要先进行一个下采样，然后这个下采样的featuremap需要和经过一个Bottleneck的featuremap的层做concat，
        # 然后这个featuremap再经过一系列的bottleneck
        downsample = None
...
class Upsample_unit(nn.Module):
    ...
    def forward(self, x, up_x):
        # 下采样结果横向转成上采样的输入，就是1×1的卷积，卷积成上采样固定的通道数
        out = self.u_skip(x)
        # 如果不是最底层的话，out需要加上下一层的的结果先线性插值，然后1×1的卷积
        if self.ind > 0:
        ...
        res = F.interpolate(res, size=self.output_shape, mode='bilinear',
                            align_corners=True)
        # 这个skip1是输出直接前一个阶段同层featuremap用1×1的核卷积，skip2是out用1×1卷积核增多一下层数
        skip1 = None
        skip2 = None
        if self.gen_skip:
            skip1 = self.skip1(x)
        ...
        # out就是上一层的输入，res是本层输出的预测结果，skip1是前一阶段下采样层的连接，skip2是前一阶段上采样层的连接，crossCov是下一阶段
        # 下采样层的输入
...
class Upsample_module(nn.Module):
    ...
    def __init__(self, output_chl_num, output_shape, chl_num=256,
        ...
        self.gen_skip = gen_skip
        self.gen_cross_conv = gen_cross_conv
        # 这里的output_chl_num是每层输出的结果的层数，应该和coco关键点数目一致
        # output_shape是每层输出的结果的shape，各层都是一致的
        # chl_num是上采样每层的维度，各层都是一样的
        # inplane是下采样相应层输入的维度
        # up_sizes是给上一层的输入的featuremap的形状
        # gen_skip是控制是否生成前一阶段到后一阶段的跳的
        # gen_cross_conv是为了控制上一阶段上采样的顶层是否卷积作为下一阶段下采样输入的
...
class Single_stage_module(nn.Module):
    ...
    def __init__(self, output_chl_num, output_shape, has_skip=False,
        ...
        # has_skip是说该阶段是否有来自前一阶段的skip作为输入
        self.has_skip = has_skip
        self.gen_skip = gen_skip
        self.gen_cross_conv = gen_cross_conv
...
class MSPN(nn.Module):
    ...
    def __init__(self, cfg, run_efficient=False, **kwargs):
        ...
        # 除了第一stage其他每一stage都有skip，除了最后stage其他每一stage都有gen_skip和crossconv
    ...
    def _calculate_loss(self, outputs, valids, labels):
        ...
        # valids应该只是为了控制这个点是否需要计算loss
        # labels就是每一层一个scale的监督，没有看到各个stage用的核
        loss1 = JointsL2Loss()
        if self.ohkm:
            loss2 = JointsL2Loss(has_ohkm=self.ohkm, topk=self.topk)
        # 如果是最后一个stage的话，就用labels[:, j+1, :, :, :]，
        # 每个stage的最后一层加上了ohkm
        loss = 0
        for i in range(self.stage_num):
            for j in range(4):
